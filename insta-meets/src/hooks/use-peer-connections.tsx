...
export function usePeerConnections({
  ...
  // Helper to clean up old and destroyed peers for a user
  const cleanupPeers = useCallback((peerId: string) => {
    ...
    if (peersToClean.length > 0) {
      ...
      peersToClean.sort((a, b) => b.createdAt - a.createdAt);

      for (let i = 0; i < peersToClean.length; i++) {
        const peerToClean = peersToClean[i];
  ...
  const createPeer = useCallback(
    (userToSignal: string, callerId: string, stream: MediaStream) => {
      ...
      const peerAlreadyExists = peersRef.current.find((p) => p.peerId === userToSignal);
      if (peerAlreadyExists) {
        console.log(`[PeerConnections] Already have an peer for ${userToSignal}, destroying old peer before creating a new one.`);
        cleanupPeers(userToSignal);
      }      


      console.log(`[PeerConnections] Creating new peer to connect with ${userToSignal} (initiator)`)
      ...
      try {
        ...
        peer.addStream(stream)
         ...
         // Log when a new ice candidate is generated
        peer.on("icecandidate", (candidate) => {
            if (candidate) {
              console.log(`[PeerConnections] ICE candidate generated for ${userToSignal}:`, candidate);
            }

            console.log(`[PeerConnections] Sending ICE candidate for ${userToSignal}`);
        ...
        peer.on("signal", (signal) => {
          console.log(`[PeerConnections] Generated signal for ${userToSignal} of type: ${signal.type}`);
          if (signal.type === "offer") {
            console.log(`[PeerConnections] Sending offer signal for ${userToSignal}`);
            socket?.emit("offer", {
              meetingId,
              userId: userToSignal,
              callerId,
        ...
        peer.on("iceStateChange", (state) => {
          ...
          console.log(`[PeerConnections] ICE gathering state changed for ${userToSignal}:`, peer.iceGatheringState)
          // If we reach failed state, destroy the peer and recreate it
          if (state === "failed") {
            console.log(`[PeerConnections] ICE connection failed for ${userToSignal}, destroying peer and recreating it`)
             peer.destroy()
  ...
  const addPeer = useCallback(
    (callerId: string, userId: string, incomingSignal: Peer.SignalData, stream: MediaStream) => { 
      ...
      // Check if we already have a peer for this user
      const peerAlreadyExists = peersRef.current.find((p) => p.peerId === callerId);
      if (peerAlreadyExists) {
        console.log(`[PeerConnections] Already have an peer for ${callerId}, destroying old peer before creating a new one.`);
      ...
      try { 
        ...
        peer.on("icecandidate", (candidate) => { 
          ...
          console.log(`[PeerConnections] Sending ICE candidate for ${callerId}`);
          socket?.emit("candidate", { 
            meetingId, 
            userId: callerId, 
            callerId: userId, 
        ...
        peer.on("signal", (signal) => { 
          console.log(`[PeerConnections] Generated signal for ${callerId} of type: ${signal.type}`)
          if (signal.type === "answer") { 
            console.log(`[PeerConnections] Sending answer signal for ${callerId}`);
            socket?.emit("answer", {
              meetingId,
              callerId,
              userId,
    ...
    [iceServers, isAudioOnlyMode, meetingId, socket, sdpTransform, safeSetPeers, cleanupPeers],
  );
  ...
  const handlePeerReconnect = useCallback(
    ...
    [isAudioOnlyMode, userId, createPeer, safeSetPeers],
  )
        ...
        if (signal.type === "answer") {
          ...
          if (pc) {
             if (pc.signalingState !== "have-local-offer") {
              console.log(`[PeerConnections] Cannot apply answer to peer ${peerId} in state ${pc.signalingState}, destroying peer`)
              cleanupPeers(peerId);
              return false
             }
          }
