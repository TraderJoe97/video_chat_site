...
export default function MeetingRoom() {
    ...
    const init = async () => {
      try {
        ...
        socketConnection.on("connect", () => {
          socketConnection.emit("ping", "Hello server");
          ...
          (data: { answer: SignalData; callerId: string }) => {
            ...
            if (peer) {
              ...
              if (pc && pc.signalingState === "have-local-offer") {
                ...
                console.warn(
                  `Answer from ${data.callerId} ignored (signalingState: ${
  ...
  const addPeer = useCallback(
    (incomingSignal: SignalData, callerId: string, stream: MediaStream) => {
      console.log(
        `Adding peer for ${callerId}, signal type:`,
        incomingSignal.type || "candidate"
      );
      const peer = new Peer({
        initiator: false,
        trickle: true, // Change to true to use ICE trickle for faster connections
      ...
      peer.on("signal", (data) => {
        console.log(
          `Sending answer to ${callerId}, type:`,
          data.type || "candidate"
        );
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      Object.values(peersRef.current).forEach((peer) => {
        peer.send(
          JSON.stringify({ type: "bandwidth", value: bandwidthRef.current })
        );
      });
    ...
    if (pc) {
      pc.oniceconnectionstatechange = () => {
        ...
        if (
          ...
          console.warn(
            `ICE connection failed with ${userId}, attempting restart`
          );
          pc.restartIce();
        }
      };
                      ...
                      <video
                        ...
                        ref={(el) => {
                          if (el) {
                            ...
                            el.play().catch((err) => {
                              console.error(
                                `Error playing video for ${userId}:`,
                                err
                              );
                              ...
                              el.play().catch((err2) =>
                                console.error(
                                  `Error playing muted video for ${userId}:`,
                                  err2
                                )
                              );
                            });
            ...
            onClick={() => {
              navigator.mediaDevices
                ...
                .catch((err) => {
                  ...
                  toast.error(
                    "Could not share screen. Please check permissions."
                  );
                });
            }}
          >
